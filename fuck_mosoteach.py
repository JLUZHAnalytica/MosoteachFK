...
def write(string):
    with open('离散数学.txt', 'a', encoding='utf-8')as f:
        f.write(string)
...
def get_QUIZ_data(web):
    ...
    title = html.xpath('//*[@id="interaction-title-box"]/div[1]/div[1]')[0].xpath('string()')
    write(title + '\n\n')
    #############问题
    ...
    for i in range(1, len(questions) + 1):
        #         print(str(i),'ti')
        flag = 0
        question_s = html.xpath(
            '//*[@id="cc-main"]/div[2]/div[4]/div[{}]/div[1]/div/div[1]/div/div[3]/pre/text()'.format(
                i))  # [0].xpath('string()')
    #         print(len(question_s))
    if len(question_s) > 1 or len(question_s) == 0:
        URL = html.xpath(
            '//*[@id="cc-main"]/div[2]/div[4]/div[{}]/div[1]/div/div[1]/div/div[3]/pre/span/img/@src'.format(i))
        if URL == []:
            Math = ''
        else:
    ...
    try:
        src = \
            html.xpath('//*[@id="cc-main"]/div[2]/div[4]/div[{}]/div[1]/div/div[1]/div/div[3]/pre/img/@src'.format(i))[
                0]
        question = question + '\n' + src
    except:
        pass
        ...
        for j in range(1, len(opts) + 1):
            count = 0
            opt = html.xpath('//*[@id="cc-main"]/div[2]/div[4]/div[{}]/div[1]/div/div[3]/div[{}]'.format(i, j))[
                0].xpath('string()').replace('\n', '').replace(' ', '')
            try:
                opt_list = html.xpath(
                    '//*[@id="cc-main"]/div[2]/div[4]/div[{}]/div[1]/div/div[3]/div[{}]/span/span'.format(i, j))
                count = 1
                if len(opt_list) > 1:
                    try:
                        URL = html.xpath(
                            '//*[@id="cc-main"]/div[2]/div[4]/div[{}]/div[1]/div/div[3]/div[{}]/span/span[{}]/img/@src'.format(
                                i, j, len(opt_list)))[0]
            ...
            try:
                src = html.xpath(
                    '//*[@id="cc-main"]/div[2]/div[4]/div[{}]/div[1]/div/div[3]/div[{}]/span[3]/img/@src'.format(i, j))[
                    0]
                opt = opt + src
            except:
                pass
            write(opt + '\n')
        #############正确答案
        answer = html.xpath('//*[@id="cc-main"]/div[2]/div[4]/div[{}]/div[2]/div[1]/div[1]'.format(i))[0].xpath(
            'string()').replace('\n', '').replace(' ', '')
        write(answer + '\n\n')
    print(title, '页面爬取成功...')
...
def main1(web, uersname, password):
    ...
    # time.sleep(1)
    passw = web.find_element_by_xpath('//*[@name="user_password"]')
    passw.send_keys('{}'.format(password))
    time.sleep(1)
...
if __name__ == '__main__':
    ...
    with open("access_file.txt", 'r') as fd:
        uersname = fd.readline()[:-1]
        password = fd.readline()
    # uersname = str(input("usrsname:"))
    ...
    time.sleep(1)
    if result == '1':
        main2(web, uersname, password)
    elif result == '2':
        main1(web, uersname, password)
    web.close()
